...
namespace mtc::FileIO {

	MTCFILEIO_ATTRBT Ret MTCFILEIO_CALL ReadString(const char* srcFileName, basic_stringstream<char16_t>& des, Endian& endian) {
		Ret ret;
		union {
	...
	MTCFILEIO_ATTRBT Ret MTCFILEIO_CALL WriteString(const char* desFileName, basic_stringstream<char16_t>& src, const Endian& endian) {
		Ret ret = Ret::Unknown;
		union {
			char c[sizeof(char16_t)];
			char16_t w;
	...
	template<> Ret MTCFILEIO_CALL Manager<basic_istream<char16_t>>::OpenStream(const char* fileName, Endian _endian) {
		Ret ret;
	...
	template<> Ret MTCFILEIO_CALL Manager<basic_istream<char16_t>>::CloseStream() {
		delete pStream;
		return Ret::OK;
	}

	template<> Ret MTCFILEIO_CALL Manager<basic_ostream<char16_t>>::OpenStream(const char* fileName, Endian _endian) {
	...
	template<> Ret MTCFILEIO_CALL Manager<basic_ostream<char16_t>>::CloseStream() {
		Ret ret;
		if (pStream == nullptr) {
	...
	template<> Ret MTCFILEIO_CALL Manager<basic_istream<char>>::OpenStream(const char* fileName, Endian _endian) {
		Ret ret;
	...
	template<> Ret MTCFILEIO_CALL Manager<basic_istream<char>>::CloseStream() {
		Ret ret;
		if (pStream == nullptr) {
			ret = Ret::NullPointerRefernce;
		}
	...
	template<> Ret MTCFILEIO_CALL Manager<basic_ostream<char>>::OpenStream(const char* fileName, Endian _endian) {
		Ret ret;
	...
	template<> Ret MTCFILEIO_CALL Manager<basic_ostream<char>>::CloseStream() {
		Ret ret;
		if (pStream == nullptr) {
			ret = Ret::NullPointerRefernce;
		}
